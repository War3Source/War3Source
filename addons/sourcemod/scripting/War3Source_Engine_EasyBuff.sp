...
public Native_War3_AddSkillBuff(Handle:plugin, numParams)
{
    ...
    for(new i = 0; i < GetArraySize(g_hSkillBuffs); i++)
    {
        ...
        {
            War3_LogInfo("EasyBuff: Skipping race %i (skill %i, buff %i) - already exists!", iRace, iSkill, buff);
            return;
        }
    }
...
public Native_War3_AddItemBuff(Handle:plugin, numParams)
{
    ...
    for(new i = 0; i < GetArraySize(g_hItemBuffs); i++)
    {
        ...
        {
            War3_LogInfo("EasyBuff: Skipping item %i (buff %i) - already exists!", iItem, buff);
            return;
        }
    }
...
ResetSkills(client, race)
{
    for(new i = 0; i < GetArraySize(g_hSkillBuffs); i++)
    {
        if(GetArrayCell(g_hBuffRace, i) == race)
        {
            ...
            War3_LogInfo("EasyBuff: Resetting buff %i for race %i!", buff, race);
...
InitSkills(client, race)
{
    for(new i = 0; i < GetArraySize(g_hSkillBuffs); i++)
    {
        if(GetArrayCell(g_hBuffRace, i) == race)
        {
            ...
            new W3Buff:buff = W3Buff:GetArrayCell(g_hSkillBuffs, i);
            new any:value = GetArrayCell(g_hBuffSkillValues, i, skill);
            
            War3_LogInfo("EasyBuff: Giving buff %i for race %i to %i with a magnitude of %f!", buff, race, client, value);
...
ResetItems(client, item)
{
    for(new i = 0; i < GetArraySize(g_hItemBuffs); i++)
    {
        if(GetArrayCell(g_hBuffItem, i) == item)
        {
            ...
            War3_LogInfo("EasyBuff: Resetting buff %i for item %i!", buff, item);
...
InitItems(client, item)
{
    for(new i = 0; i < GetArraySize(g_hItemBuffs); i++)
    {
        if(GetArrayCell(g_hBuffItem, i) == item)
        {
            ...
            War3_LogInfo("EasyBuff: Giving buff %i for item %i to %i with a magnitude of %f!", buff, item, client, value);
