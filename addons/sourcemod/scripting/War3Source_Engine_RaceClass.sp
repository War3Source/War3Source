...
new Handle:g_hOnWar3RaceAdded; //other races backwards compatable


new bool:g_bAfterPluginReady = false;
...
public bool:InitNativesForwards()
{
    ...
    g_hOnWar3RaceAdded = CreateGlobalForward("OnRaceAdded", ET_Ignore, Param_Cell, Param_Cell);
...
CreateRaceEnd(raceid){
    ...
    if(raceid>0){
        ...
        if(!ignoreRaceEnd&&raceid>0 && ReloadRaces_Id[raceid]==false)  // Dont let reload races over write these variables.
        {
			...
			Call_StartForward(g_hOnWar3RaceAdded);
			Call_PushCell(raceid);
			Call_PushCell(g_bAfterPluginReady);
			Call_Finish();
