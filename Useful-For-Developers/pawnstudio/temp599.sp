...
//leap
new Float:leapPower[5]={0.0,1.5,1.7,1.9,2.1};
...
{
	if ((buttons & IN_JUMP)&&War3_GetGame() != Game_CS ) //assault for non CS games
	{
		...
		if (race==thisRaceID)
		{
			...
			if(sl>0&&SkillAvailable(client,thisRaceID,SKILL_LEAP))
			{
				...
				*/
				...
				War3_CooldownMGR(client,10.0,thisRaceID,SKILL_ASSAULT,_,_,false);
