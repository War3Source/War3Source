			...
			if (skilllevelheadhunter > 0 && dmgamount > 0 && !War3_GetImmunity(victim,Immunity_Skills))
			{
				...
				else
				{
					...
					if(GetRandomInt(1,100)<=percent)
					{
						...
						totaldamage += damage;
						PrintToConsole(attacker,"[Head Hunter] You inflicted +%d Damage",damage);
					}
				}
...
{
	if(War3_ValidPlayer(client,true)&&pressed && race==thisRaceID)
	{
