...
bool:InitNativesForwards()
{
	...
	CreateNative("W3GetBuffSum",NW3GetBuffSum);
	CreateNative("W3ResetAllBuff",NW3ResetAllBuff);
	CreateNative("W3ResetBuff",NW3ResetBuff);
	return true;
}
...
public NW3GetBuffSum(Handle:plugin,numParams) {
	
	return GetBuffSum(GetNativeCell(1),GetNativeCell(2)); //returns float usually
}
...
public NW3ResetBuff(Handle:plugin,numParams) {
	new client=GetNativeCell(1);
...
//		armor=-10.0;
	...
	//PrintToChatAll("%f",(1-(armor*0.06)/(1+armor*0.06)));
...
{	
	if(FindDataMapOffs(index,"m_flGravity")>-1){
		SetEntityRenderMode(index,RENDER_TRANSCOLOR);
