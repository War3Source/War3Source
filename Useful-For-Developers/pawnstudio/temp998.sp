...
{	
	...
	if(query!=INVALID_HANDLE){
		...
		if(killstoinsert>0)
		{
			Format(KillInsertQuery,9999,"%s ON DUPLICATE KEY UPDATE kills=kills+1",KillInsertQuery);
			SQL_TQuery(hdatabase,SQLTProcessEventEnd,KillInsertQuery);
		}
		if(deathstoinsert>0)
		{
			Format(DeathInsertQuery,9999,"%s ON DUPLICATE KEY UPDATE deaths=deaths+1",DeathInsertQuery);
			SQL_TQuery(hdatabase,SQLTProcessEventEnd,DeathinsertQuery);
		}
