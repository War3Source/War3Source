...
CooldownResetByCRS(client,raceid,skillnum){
	if(CooldownPointer[client][raceid][skillnum]>0){
		Cooldown[CooldownPointer[client][raceid][skillnum]][cexpiretime]=GetGameTime();
	}
}
...
public Internal_PrintSkillNotAvailable(cooldownindex){
	...
	Format(skillname,32,"%s",Cooldown[cooldownindex][cskillname]);
	if(ValidPlayer(client)){
		PrintHintText(client,"%s Is Not Ready. %d Seconds Remaining.",strlen(skillname)>0?skillname:War3_IsSkillUltimate(race,skill)?"Ultimate":"Ability",War3_CooldownRemaining(client,race,skill));
	}
}
