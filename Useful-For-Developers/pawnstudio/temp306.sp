...
public PlayerHurtEvent(Handle:event,const String:name[],bool:dontBroadcast)
{
	...
	if(victimuserid&&attackeruserid&&victimuserid!=attackeruserid) //damage is not self dealt
	{
		...
		if(race_victim==thisRaceID && skilllevel>0 && GetClientHealth(victim)>0) //if this person is actually our race...and has a level in our skill of interest
		{
			...
			if(GetRandomFloat(0.0,1.0)<=RewindChance[skilllevel] && !War3_GetImmunity(attacker,Immunity_Skills)) //chance roll, and attacker isnt immune to skills
			{
		...
		{
			if(GetRandomFloat(0.0,1.0)<=BashChance[skilllevel] && !War3_GetImmunity(victim,Immunity_Skills) && !bIsBashed[victim])
			{
...
public OnGameFrame() //this is a sourcemod forward?, every game frame it is called. forwards if u implement it sourcemod will call you
{
	for(new i=1;i<=MaxClients;i++){//MaxClients is a variable assigned by sourcemod depending on the slot count of the server so ur looping from 1 client to all clients? yesk
		if(War3_ValidPlayer(i,true))//valid (in game and shit) and alive (true parameter)k
		{
			if(RewindHPAmount[i]>0){
				...
				{
					//War3_FlashScreen(victim,RGBA_COLOR_RED,0.2,_);  // _ parameter means use defautl value provided
					
					//just for demonstation u wana flash blue
					War3_FlashScreen(i,{0,255,20,150},0.2,_);
					///or u wana define ur own damn COLOR
					RewindLastPrintScreen[i]=GetGameTime();
				}
			}
		}
